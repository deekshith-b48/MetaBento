...
```sql
-- Run the contents of database/schema.sql first
```

### 2. **Enhanced Schema** (new features)
```sql
...
CREATE INDEX IF NOT EXISTS idx_profile_cards_visibility ON profile_cards(visibility);
CREATE INDEX IF NOT EXISTS idx_profile_cards_order ON profile_cards(user_id, order_index);
...
CREATE INDEX IF NOT EXISTS idx_qr_activities_scanner ON qr_activities(scanner_user_id);
CREATE INDEX IF NOT EXISTS idx_qr_activities_scanned ON qr_activities(scanned_user_id);
CREATE INDEX IF NOT EXISTS idx_qr_activities_timestamp ON qr_activities(scan_timestamp);
CREATE INDEX IF NOT EXISTS idx_networking_events_active ON networking_events(is_active);
CREATE INDEX IF NOT EXISTS idx_networking_events_time ON networking_events(start_time);
...
DROP TRIGGER IF EXISTS update_user_level_trigger ON user_levels;
...
CREATE OR REPLACE VIEW active_profile_cards AS
SELECT 
    pc.*,
    u.username,
    u.is_public
...
## 4. **Test Sample Data** (Optional)

```sql
-- Insert sample networking event
INSERT INTO networking_events (
    ...
    'Web3 Networking Mixer',
    'Connect with fellow Web3 builders and creators',
    'networking',
...
-- Test the award_user_xp function
SELECT award_user_xp(
    (SELECT id FROM users LIMIT 1),
    25,
    'test_award'
);
...
✅ **Your database is now ready for the enhanced MetaBento backend!**
